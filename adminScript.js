showForm = buttontype => {
  ...
  var image = `<input type="file" id="img" name="img" style=";" accept="image/*">`;
  ...
  document.getElementById(
   ...
   <div class = "gridItem"><label style='font-size:15px; text-color: black;'>Date:</label><input type='date' name ="Date"></div>
  ...
  document.getElementById('addForm').style.height = '400px';
...
changeData = () => {
  ...
  if (buttontype === 'buttonadd') {
    if (validator) {
      ...
      let image =
        '/images/' + document.forms['AdminForm']['img'].value.split('\\')[2];
    ...
    for (var i = 0; i < localStorage.length; i++) {
      if (localStorage.key(i).startsWith(salon_name)) {
        ...
        saloon_remove.push(localStorage.key(i));
      }
    }
    for (let i = 0; i < saloon_remove.length; i++) {
      localStorage.removeItem(saloon_remove[i]);
    }
